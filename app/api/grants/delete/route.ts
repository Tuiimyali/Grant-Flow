...
export async function POST(req: NextRequest) {
  ...
  if (!ownedIds.length) {
    return NextResponse.json({ error: 'No matching grants found for your organization' }, { status: 404 })
  }
  ...
  const { error: matchErr } = await admin.from('grant_matches').delete().in('grant_id', ownedIds)
  if (matchErr) console.error('[delete] grant_matches:', matchErr.message)

  const { error: pipeErr } = await admin.from('pipeline_items').delete().in('grant_id', ownedIds)
  if (pipeErr) console.error('[delete] pipeline_items:', pipeErr.message)

  const { error: grantsErr } = await admin.from('grants').delete().in('id', ownedIds)
  if (grantsErr) console.error('[delete] grants:', grantsErr.message)

  if (grantsErr || pipeErr) {
    return NextResponse.json({ error: 'Delete partially failed — check server logs' }, { status: 500 })
  }
