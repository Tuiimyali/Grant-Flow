...
export default function GrantDetailPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [{ data: full }, { data: detail }, { data: match }] =
        await Promise.all([
          supabase.from('grants_full').select('*').eq('id', id).single(),
          supabase
            .from('grants')
            .select(
              'sections, attachments, description, source_url, review_criteria, requirements_summary'
            )
            .eq('id', id)
            .single(),
          supabase
      ...
      setBreakdown(
        (match as { score_breakdown: FitBreakdown | null } | null)
  ...
  function updateSection(
    ...
    setSections((prev) =>
      prev.map((s) => (s._id === _id ? { ...s, [field]: value } : s))
    )
  }
  ...
  function updateAttachment(_id: string, value: string) {
    setAttachments((prev) =>
      prev.map((a) => (a._id === _id ? { ...a, name: value } : a))
    )
  }
  ...
  const amountStr =
    g.amount_low != null && g.amount_high != null
      ...
      : g.amount_high != null
        ? `Up to ${formatCurrency(g.amount_high, { compact: true })}`
        : g.amount_low != null
          ? `From ${formatCurrency(g.amount_low, { compact: true })}`
          : null
    ...
    >
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              <span
                className={`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ${status.bg} ${status.text}`}
              >
                {status.label}
              </span>
              {g.is_renewal && (
            ...
            {g.funder && (
              <p className="text-sm text-slate-500 mt-0.5">{g.funder}</p>
            )}
          </div>
          ...
          <div className="flex items-center gap-3 flex-wrap shrink-0">
            {g.fit_score != null && (
              <span
                className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold tabular-nums ${fitColor.bg} ${fitColor.text} ${fitColor.border}`}
              >
                {g.fit_score}% fit
              </span>
            )}
      ...
      <div className="flex-1 p-6 grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-6 items-start">
        ...
        <div className="space-y-4">
          ...
          <Card title="Overview">
            <dl className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
              ...
              <Fact label="Effort">
                {g.effort_weeks != null ? `${g.effort_weeks} weeks` : '—'}
              </Fact>
              {amountStr && (
                <Fact label="Amount" wide>
                  {amountStr}
                </Fact>
              )}
              {sourceUrl && (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-left border-b border-slate-100">
                      <th className="pb-2 pr-4 text-[10px] font-semibold uppercase tracking-wide text-slate-400 w-2/5">
                        Criterion
                      </th>
                      <th className="pb-2 pr-4 text-[10px] font-semibold uppercase tracking-wide text-slate-400 w-16">
                        Weight
                      </th>
                      <th className="pb-2 text-[10px] font-semibold uppercase tracking-wide text-slate-400">
                        Description
                      </th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50">
                    {reviewCriteria.map((c, i) => (
                      <tr key={i} className="align-top">
                        ...
                        <td className="py-2.5 text-slate-500 leading-relaxed">
                          {c.description || '—'}
                        </td>
                      </tr>
                    ))}
          ...
          {(breakdown || g.fit_score != null) && (
            <Card title="Fit Score Breakdown">
              <div className="space-y-3">
                {breakdown ? (
                  (Object.keys(breakdown) as (keyof FitBreakdown)[]).map(
                    (dim) => {
                  ...
                  <p className="text-xs text-slate-400">
                    Breakdown not yet calculated. Save your organisation profile
                    to generate scores.
                  </p>
                )}
                ...
                {g.fit_score != null && (
                  <div className="pt-2 mt-2 border-t border-slate-100 flex items-center justify-between">
                    ...
                    <span
                      className={`inline-flex rounded-full border px-2 py-0.5 text-xs font-semibold tabular-nums ${fitColor.bg} ${fitColor.text} ${fitColor.border}`}
                    >
                      {g.fit_score}%
                    </span>
                  </div>
        ...
        <div className="space-y-4">
          {/* Sections builder */}
          <Card
            title={`Application Sections${sections.length ? ` (${sections.length})` : ''}`}
            action={
              <button
                onClick={addSection}
              ...
              <div className="space-y-2">
                {sections.map((s, i) => (
                  <div key={s._id} className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-xs text-slate-400 w-4 shrink-0 text-right tabular-nums">
                      {i + 1}.
                    </span>
                    <input
                      type="text"
                    ...
                    <input
                      ...
                      value={s.page_limit}
                      onChange={(e) =>
                        updateSection(s._id, 'page_limit', e.target.value)
                      }
                      placeholder="pp"
          ...
          {/* Attachments builder */}
          <Card
            title={`Required Attachments${attachments.length ? ` (${attachments.length})` : ''}`}
            action={
              <button
          ...
          >
            {attachments.length === 0 ? (
              <p className="text-xs text-slate-400 py-1">
                No attachments yet. List required documents (letters, budgets,
                etc.).
              </p>
            ) : (
              <div className="space-y-2">
