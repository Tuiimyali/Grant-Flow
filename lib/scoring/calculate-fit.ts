...
function scoreEligibility(
  ...
  const sovereigntyMatch =
    !!org.sovereignty_status &&
    allowedSovereignty.includes(org.sovereignty_status)
...
function scoreBudget(
  ...
  if (ratio >= 0.1 && ratio <= 0.3) return 100 // sweet spot: 10–30% of budget
  if (ratio > 0.3 && ratio <= 0.5) return 80 // acceptable
  if (ratio < 0.1 && ratio >= 0.05) return 70 // small grant, fine
  if (ratio < 0.05) return 60 // very small, low ROI
  if (ratio > 0.5 && ratio <= 0.75) return 40 // >50% of budget, lower score
  if (ratio > 0.75 && ratio <= 1.0) return 20 // >75% of budget, risky
...
function scoreCapacity(org: Partial<OrganizationProfile>): number {
  ...
  if (
    org.single_audit_status === 'current' ||
    org.single_audit_status === 'not_required'
  )
    points += 20
