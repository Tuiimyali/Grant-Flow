        ...
        data.forEach((raw, i) => {
          ...
          if (rawHigh) {
            ...
            if (isNaN(n))
              rowErrors.push({
                row: rowNum,
                field: 'amount_high',
                message: `Invalid number: "${rawHigh}"`,
              })
            else amount_high = n
          }
          if (rawEff) {
          ...
          if (rawDate) {
            ...
            if (isNaN(d.getTime())) {
              ...
              deadline = rawDate.match(/^\d{4}-\d{2}-\d{2}$/)
                ? rawDate
          ...
          const is_renewal =
            rawRenewal === 'true' || rawRenewal === '1' || rawRenewal === 'yes'
          ...
          const initial_status = VALID_STATUSES.includes(rawStatus)
            ? rawStatus
            : 'discovered'
