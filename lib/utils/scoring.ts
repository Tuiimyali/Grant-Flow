...
function mapEligibilityToSovereignty(eligibility: string[]): string[] {
  ...
  for (const e of eligibility) {
    const key = e.toLowerCase()
    for (const [pattern, statuses] of Object.entries(
      ELIGIBILITY_TO_SOVEREIGNTY
    )) {
...
function categoryFocusScore(
  ...
  for (const area of orgFocus) {
    ...
    if (cat.includes(al) || al.includes(cat)) {
      best = 1
      break
    }
    // token overlap
...
function geoScore(
  ...
  return shared > 0
    ? Math.min(shared / Math.max(orgTok.length, grantTok.size), 1)
    : 0
}
...
export function calcFitScore(
  ...
  const raw = {
    ...
    sovereignty: includesScore(
      org.sovereignty_status,
      grant.eligible_sovereignty
    ),
  ...
  return {
    ...
    band: fitBand(Math.round(composite * 100)),
    breakdown,
  }
}
...
export function calcFitScoreFromGrantRow(
  ...
  const eligibleOrgTypes = mapEligibilityToOrgTypes(
    grant.eligibility_types ?? []
  )
  const eligibleSovereignty = mapEligibilityToSovereignty(
    grant.eligibility_types ?? []
  )
  ...
  const criteria: GrantCriteria = {
    ...
    eligible_sovereignty: eligibleSovereignty.length
      ? eligibleSovereignty
      : null,
  ...
  const rawSov = includesScore(
    org.sovereignty_status,
    criteria.eligible_sovereignty
  )
...
export const FIT_COLORS: Record<
  FitBand,
