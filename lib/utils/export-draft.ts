...
export async function exportDraftAsDocx(
  ...
  const titlePage: Paragraph[] = [
    ...
    new Paragraph({
      children: [
        new TextRun({ text: grantName, bold: true, size: 48, color: '1e293b' }),
      ],
    ...
    deadline
      ? new Paragraph({
          children: [
            new TextRun({
              text: `Deadline: ${formatDeadlineForDoc(deadline)}`,
              size: 24,
              color: '64748b',
            }),
          ],
    ...
    sections: [
      {
        ...
        headers: {
          default: new Header({
            children: [
              new Paragraph({
                children: [
                  new TextRun({ text: grantName, size: 18, color: '94a3b8' }),
                ],
        ...
        children: [...titlePage, ...tocSection, ...sectionPages],
      },
    ],
  })

  const blob = await Packer.toBlob(doc)
  const filename = `${grantName
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-|-$/g, '')}-application.docx`
