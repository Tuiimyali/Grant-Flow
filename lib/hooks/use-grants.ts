...
export function useGrants(): UseGrantsResult {
  ...
  const updateStatus = useCallback(
    async (grantId: string, status: string) => {
      // Optimistic update
      setGrants((prev) =>
        prev.map((g) =>
          g.id === grantId ? { ...g, pipeline_status: status } : g
        )
      )
      ...
      console.log('[useGrants] status update response:', {
        updated,
      ...
      if (error) {
        ...
        console.warn(
          '[useGrants] update matched 0 rows — trying insert into pipeline_items'
        )
        const { data: inserted, error: insertErr } = await supabase
          .from('pipeline_items')
          .insert({ grant_id: grantId, status })
          .select('id, grant_id, status')
        console.log('[useGrants] insert response:', {
          inserted,
