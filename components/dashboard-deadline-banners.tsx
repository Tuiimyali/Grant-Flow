...
function Banner({
  variant, icon, message, linkHref, linkLabel, onDismiss,
}: {
  ...
  const border = variant === 'danger' ? 'var(--danger-border)' : 'var(--warning-border)'
  ...
  return (
    ...
    >
      ...
      <p className="flex-1 text-sm font-medium" style={{ color }}>{message}</p>
      <Link href={linkHref} className="text-xs font-semibold underline shrink-0" style={{ color }}>
        {linkLabel}
      </Link>
      <button
        onClick={onDismiss}
