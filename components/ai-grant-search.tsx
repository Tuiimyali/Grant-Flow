...
const FUNDER_TYPE_CONFIG: Record<FunderType, { label: string; cls: string }> = {
  foundation:      { label: 'Foundation',      cls: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' },
  government:      { label: 'Government',      cls: 'bg-sky-500/10     text-sky-400     border-sky-500/20'     },
  corporate:       { label: 'Corporate',       cls: 'bg-violet-500/10  text-violet-400  border-violet-500/20'  },
  community:       { label: 'Community',       cls: 'bg-teal-500/10    text-teal-400    border-teal-500/20'    },
  impact_investor: { label: 'Impact Investor', cls: 'bg-amber-500/10   text-amber-400   border-amber-500/20'   },
}
...
const FLEXIBILITY_CONFIG: Record<Flexibility, { label: string; cls: string }> = {
  unrestricted:         { label: 'Unrestricted', cls: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' },
  partially_restricted: { label: 'Partial',      cls: 'bg-amber-500/10   text-amber-400   border-amber-500/20'   },
  restricted:           { label: 'Restricted',   cls: 'bg-white/5        text-[var(--text-dim)]   border-white/10' },
}
...
function StatusBadge({ status }: { status: string }) {
  const cls =
    status === 'awarded'     ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
    status === 'submitted'   ? 'bg-sky-500/10     text-sky-400     border-sky-500/20'     :
    status === 'writing'     ? 'bg-violet-500/10  text-violet-400  border-violet-500/20'  :
    status === 'researching' ? 'bg-amber-500/10   text-amber-400   border-amber-500/20'   :
                               'bg-white/5        text-[var(--text-dim)] border-white/10'
  return (
...
function ExistingGrantCard({ grant: g }: { grant: GrantsFullRow }) {
  ...
  return (
    <Link
      href={`/grants/${g.id}`}
      className="group flex flex-col gap-2.5 rounded-xl p-4 transition-all"
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-1.5 mt-auto">
        ...
        {amountStr && <span className="text-xs" style={{ color: 'var(--text-dim)' }}>{amountStr}</span>}
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <p className="text-sm font-semibold leading-snug line-clamp-2" style={{ color: 'var(--text-primary)' }}>{s.name}</p>
        <p className="text-xs truncate mt-0.5" style={{ color: 'var(--text-dim)' }}>{s.funder}</p>
      </div>

      {/* Description */}
      <p className="text-xs leading-relaxed line-clamp-3" style={{ color: 'var(--text-secondary)' }}>{s.description}</p>
      ...
      {s.why_match && (
        <div className="rounded-lg border px-3 py-2 text-xs leading-relaxed"
          style={{ borderColor: 'var(--gold-border)', backgroundColor: 'var(--gold-bg)', color: 'var(--text-secondary)' }}>
      ...
      {s.how_to_find && (
        <div className="rounded-lg border px-3 py-2 text-xs leading-relaxed"
          style={{ borderColor: 'rgba(56,189,248,0.2)', backgroundColor: 'rgba(56,189,248,0.06)', color: 'var(--text-secondary)' }}>
          <span className="font-semibold text-sky-400">How to find it: </span>
          {s.how_to_find}
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center gap-2 mt-auto pt-1">
        {added ? (
          ...
          <button type="button" onClick={handleAdd} disabled={adding}
            className="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold disabled:opacity-60 transition-opacity btn-scale"
...
function filterSuggestions(suggestions: GrantSuggestion[], filter: SuggestionFilter): GrantSuggestion[] {
...
export default function AiGrantSearch({ grants, onAdded }: AiGrantSearchProps) {
  ...
  async function handleSearch() {
    ...
    setLoading(true); setError(null); setResults(null); setActiveFilter('all')
    ...
    try {
      ...
      if (!res.ok || json.error) { setError(json.error ?? 'Search failed. Please try again.'); return }
  ...
  async function handleAddSuggestion(s: GrantSuggestion): Promise<boolean> {
    ...
    if (rpcErr) { toast(`Failed to add grant: ${rpcErr.message}`, 'error'); return false }

    toast(`Added "${s.name}" to pipeline`, 'success')
    if (newGrantId) recalculateGrantScore(supabase, organizationId, newGrantId).catch(console.error)
    onAdded?.()
    return true
  }

  const matchedGrants = results
    ? results.existing_matches.map(id => grants.find(g => g.id === id)).filter((g): g is GrantsFullRow => g !== undefined)
    : []
  ...
  return (
    <div style={{ borderBottom: '1px solid var(--border)', background: 'linear-gradient(to bottom, var(--surface) 0%, var(--bg) 100%)' }}>
      ...
      <div className="px-6 pt-5 pb-4">
        <div className="max-w-3xl">
          ...
          <div className="flex items-center gap-2.5 mb-3">
            ...
            <div>
              <span className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>AI Grant Search</span>
          ...
          <div className="flex gap-2.5">
            <textarea
              ...
              rows={2}
              disabled={loading}
              className="flex-1 rounded-xl px-4 py-3 text-sm focus:outline-none resize-none leading-relaxed disabled:opacity-50"
              style={{
            ...
            <button
              ...
              className="shrink-0 inline-flex items-center gap-2 rounded-xl px-5 text-sm font-semibold disabled:opacity-50 transition-opacity self-stretch btn-scale"
              style={{ backgroundColor: 'var(--gold)', color: '#0C0C0E' }}
            >
              {loading ? (
                <>
          ...
          {!results && !loading && (
            <div className="mt-3 flex flex-wrap items-center gap-2">
              {recentSearches.length > 0 ? (
                <>
                  <span className="text-[11px] font-medium uppercase tracking-wide" style={{ color: 'var(--text-dim)' }}>Recent:</span>
                ...
                <>
                  <span className="text-[11px] font-medium uppercase tracking-wide" style={{ color: 'var(--text-dim)' }}>Try:</span>
      ...
      {loading && (
        <div className="px-6 pb-5">
          <div className="flex items-center gap-2.5 text-sm" style={{ color: 'var(--text-dim)' }}>
            <svg className="animate-spin w-4 h-4 shrink-0" style={{ color: 'var(--gold)' }} viewBox="0 0 24 24" fill="none">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 0 1 8-8V0C5.373 0 0 5.373 0 12h4Z" />
            </svg>
          ...
          <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="rounded-xl p-4 space-y-2.5 animate-pulse"
                style={{ backgroundColor: 'var(--surface-2)', border: '1px solid var(--border)' }}>
                <div className="flex gap-1.5 mb-1">
                  <div className="h-5 rounded-full w-20 bg-emerald-500/10" />
                  <div className="h-5 rounded-full w-16 bg-amber-500/10" />
                </div>
                <div className="h-4 rounded w-3/4" style={{ backgroundColor: 'var(--surface-3)' }} />
                <div className="h-3 rounded w-1/2" style={{ backgroundColor: 'var(--surface-3)' }} />
                <div className="h-3 rounded w-full" style={{ backgroundColor: 'var(--surface-3)' }} />
                <div className="h-3 rounded w-5/6" style={{ backgroundColor: 'var(--surface-3)' }} />
              </div>
            ))}
          </div>
        </div>
      ...
      {hasResults && (
        <div className="px-6 pb-6 space-y-5">
          ...
          <div className="flex items-start gap-2 rounded-lg px-3 py-2.5 text-xs leading-relaxed"
            style={{ backgroundColor: 'var(--surface-2)', border: '1px solid var(--border)', color: 'var(--text-dim)' }}>
            <svg className="w-3.5 h-3.5 shrink-0 mt-0.5" style={{ color: 'var(--text-dim)' }} fill="none" viewBox="0 0 24 24"
          ...
          {matchedGrants.length > 0 && (
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {matchedGrants.map(g => <ExistingGrantCard key={g.id} grant={g} />)}
              </div>
            </div>
          )}
          ...
          {allSuggestions.length > 0 && (
            <div>
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-[11px] font-semibold uppercase tracking-widest" style={{ color: 'var(--text-dim)' }}>
                  ...
                  return (
                    <button
                      ...
                      style={active
                        ...
                        : { borderColor: 'var(--border)', backgroundColor: 'var(--surface-2)', color: 'var(--text-dim)' }
                      }
                    >
                      {f.label}
                      <span className="text-[10px] font-bold" style={{ color: active ? 'var(--gold)' : 'var(--text-dim)' }}>
                        {count}
                      </span>
                    </button>
                  )
