...
const ELIGIBILITY_MAP: { keywords: string[]; label: string }[] = [
  ...
  {
    keywords: ['indigenous-led', 'native-led', 'indigenous led', 'native led'],
    label: 'Indigenous-Led Org',
  },
  {
    keywords: ['501(c)(3)', '501c3', 'nonprofit', 'non-profit'],
    label: 'Nonprofit 501(c)(3)',
  },
  {
    keywords: ['faith-based', 'faith based', 'religious organization'],
  ...
  {
    keywords: ['rural community', 'rural organization'],
...
const CATEGORY_MAP: { keywords: string[]; category: string }[] = [
  ...
  {
    keywords: ['natural resource', 'fisheries', 'soil', 'land management'],
...
function FunderTypeBadge({ type }: { type: FunderType | undefined }) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[11px] font-medium ${cfg.cls}`}
    >
      {cfg.label}
    </span>
  )
...
const FLEXIBILITY_CONFIG: Record<Flexibility, { label: string; cls: string }> =
  {
    ...
    restricted: {
      label: 'Restricted',
      cls: 'bg-white/5        text-[var(--text-dim)]   border-white/10',
    },
  }

function FlexibilityBadge({
  flexibility,
}: {
...
function FitBadge({ score }: { score: number | null | undefined }) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[11px] font-semibold tabular-nums ${cls}`}
    >
      {score}% fit
    </span>
  )
...
function filterSuggestions(
  ...
  if (filter === 'unrestricted')
    return suggestions.filter((s) => s.flexibility === 'unrestricted')
...
export default function AiGrantSearch({ grants, onAdded }: AiGrantSearchProps) {
  ...
  function saveRecentSearch(q: string) {
    const next = [q, ...recentSearches.filter((s) => s !== q)].slice(
      0,
      MAX_RECENT
    )
  ...
  async function handleAddSuggestion(s: GrantSuggestion): Promise<boolean> {
    ...
    if (newGrantId)
      recalculateGrantScore(supabase, organizationId, newGrantId).catch(
        console.error
      )
    onAdded?.()
    return true
  ...
  const hasResults =
    results && (matchedGrants.length > 0 || allSuggestions.length > 0)
          ...
          <div className="flex items-center gap-2.5 mb-3">
            ...
            <div>
              ...
              <span
                ...
                style={{ color: 'var(--text-dim)' }}
              >
                Recommends funding directions to investigate — matches your
                pipeline and suggests new leads.
              </span>
            </div>
          </div>
      ...
      {hasResults && (
        <div className="px-6 pb-6 space-y-5">
          ...
          >
            ...
            These are recommended directions based on your profile and query.
            Verify current deadlines and availability before applying.
          </div>
          ...
          {allSuggestions.length > 0 && (
            <div>
              ...
              {shownSuggestions.length > 0 ? (
                ...
                >
                  No{' '}
                  {activeFilter === 'unrestricted'
                    ? 'unrestricted'
                    : activeFilter}{' '}
                  grants in these results.
                </p>
              )}
            </div>
          )}
